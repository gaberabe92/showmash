...
from controllers import face_mash
...
def get_shows():
    ...
    return jsonify(shows=face_mash.get_all_show())


@app.route("/new_players", methods=["POST"])
def new_players():
    """"Route to return random two players"""
    return jsonify(selected_players=face_mash.select_players())
...
def vote():
    ...
    if not winner_id or not looser_id :
        return jsonify(status=False,
                       ...
                       losser_elo=0,
                       message="Please vote.")
    win_elo, lose_elo = face_mash.vote_handler(winner_id, looser_id)
    return jsonify(status=False,
                   winner_elo=win_elo,
                   losser_elo=lose_elo)
